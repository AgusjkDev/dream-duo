...
export default function Testimonials() {
    return (
        <section className="flex justify-center bg-background py-16">
            <Carousel
                opts={{ align: "center" }}
                className="w-2/3 max-w-xs md:w-10/12 lg:max-w-screen-lg md:max-w-screen-md"
            >
                <CarouselPrevious />

                <CarouselContent className="xl:-ml-8">
                    {TESTIMONIALS.map(({ key, title, description }) => (
                        <CarouselItem key={key} className="lg:basis-1/3 md:basis-1/2 xl:pl-8">
